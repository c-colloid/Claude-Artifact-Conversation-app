...
const MultiCharacterChat=()=> {
  ...
  const [msgType, sMsgType]=useState('user');
  const [nextSpk, sNextSpk]=useState(null);
  const [prefill, sPrefill]=useState('');
  const [loading, sLoading]=useState(false);
  const [error, setError]=useState('');
  const [models, setModels]=useState([]);
  const [model, setMdl]=useState('claude-sonnet-4-5-20250929');
  const [loadMdl, sLoadMdl]=useState(false);
  const [thinkOn, sThinkOn]=useState(false);
  const [thinkBdg, sThinkBdg]=useState(2000);
  const [sThink, sShowThink]=useState({});
  const [edIdx, sEdIdx]=useState(null);
  ...
  const [visMsgCnt, sVisCnt]=useState(100);
  const [confirmDlg, sCfmDlg]=useState(null);
  ...
  const getIconForModel=(dName, mId)=> {
    const name=(dName||mId).toLowerCase();
    if(name.includes('opus')) return '👑';
    if(name.includes('sonnet')) return '⭐';
    if(name.includes('haiku')) return '⚡';
    return '🤖';
  };
  const getShortName=(dName, mId)=> {
    if(dName) {
      return dName.replace('Claude ', '');
    }
    if(mId.includes('opus')) {
      if(mId.includes('4-1')) return 'Opus 4.1';
      if(mId.includes('4')) return 'Opus 4';
    }
    if(mId.includes('sonnet')) {
      if(mId.includes('4-5')) return 'Sonnet 4.5';
      if(mId.includes('4')) return 'Sonnet 4';
    }
    if(mId.includes('haiku')) {
      if(mId.includes('4-5')) return 'Haiku 4.5';
      if(mId.includes('4')) return 'Haiku 4';
    }
    return mId;
  };
  ...
  const parseResp=(responseText, conversation, thinkCont, respGrpId=null)=> {
    const messages=[];
  ...
  const updChar=useCallback((charId, updates)=> {
  ...
  const updConv=useCallback((convId, updates)=> {
  ...
  const buildSysP=useCallback((conversation, nextSpeakerId=null, messages=[])=> {
  ...
  const forkConv=useCallback((convId, msgIdx)=> {
  ...
  const delConv=useCallback((convId)=> {
    ...
    sCfmDlg({
      ...
      confirm: ()=> {
        sConvs(prev=> prev.filter(c=> c.id !==convId));
        if(curConvId===convId) {
  ...
  }, [conversations, curConvId, newConv]);
  const createCharacterGroup=(name, cIds)=> {
    const newGroup={
      id: genId(),
      name,
      cIds,
  ...
  const applyCharacterGroup=(groupId)=> {
    ...
    const newParticipantIds=[...new Set([...currentConv.partIds, ...group.cIds])];
    updConv(curConvId, {
  ...
  const getConversationStats=()=> {
    ...
    currentConv.messages.forEach((msg, index)=> {
      ...
      } else if(msg.type==='character'&&msg.charId) {
        stats.charMsgs[msg.charId]=(stats.charMsgs[msg.charId]||0)+1;
        if(!affectionTracker[msg.charId]) {
          affectionTracker[msg.charId]=50;
          stats.characterAffectionHistory[msg.charId]=[];
          stats.characterAffectionHistory[msg.charId].push({
            msgIdx: index,
            affection: 50
          });
        }
        if(msg.affection !==undefined) {
          affectionTracker[msg.charId]=msg.affection;
        }
        stats.characterAffectionHistory[msg.charId].push({
          msgIdx: index,
          affection: affectionTracker[msg.charId]
        });
      }
    });
        ...
        if(data.conversation&&data.characters) {
          ...
          const newConv={
            ...
            messages: data.conversation.messages.map(msg=> ({
              ...msg,
              charId: msg.charId ? (charIdMap[msg.charId] ?? msg.charId) : null,
              ts: getTs()
            })),
            ...mkTs()
          };
  ...
  const generateResponse=async (messages, usePrefill=false, customPrefill=null, forcedNextSpeaker=null)=> {
    ...
    try {
      ...
      const sysPrompt=buildSysP(conversation, forcedNextSpeaker, messages);
      const participants=conversation.partIds
        .map(id=> getCharById(id))
        .map(c=> getEffectiveCharacter(c))
        .filter(c=> c);
      ...
      const sanitizedMessages=messages
        ...
        .map(msg=> {
          ...
          if(msg.type==='narration') {
            ...
            const char=getCharById(msg.charId);
      ...
      let prefillToUse=customPrefill !==null ? customPrefill : (usePrefill ? prefill : '');
      ...
      const { messages: parsedMessages, characterUpdates }=parseResp(fullContent, conversation, thinkCont, respGrpId);
  ...
  const hSend=useCallback(async ()=> {
    ...
    await generateResponse(newHistory, true, null, nextSpk);
    sNextSpk(null);
  }, [prompt, curConvId, msgType, nextSpk, getCurMsgs, updConv, generateResponse]);
  const hEdit=useCallback((index)=> {
    const message=getAllMsgs[index];
    sEdIdx(index);
    sEdCont(message.content);
    sEdEmo(message.emotion||null);
    sEdAff(message.affection !==undefined&&message.affection !==null ? message.affection : null);
  }, [getAllMsgs]);
  const hSave=useCallback((index)=> {
    const currentMessages=getAllMsgs;
    const upd=[...currentMessages];
    upd[index].content=edCont;
    ...
    sEdEmo(null);
    sEdAff(null);
  }, [getAllMsgs, edCont, editEmo, editAff, curConvId, updConv]);
  ...
  const hRegenGrp=useCallback(async (index)=> {
    ...
    if(targetMessage.type==='narration') {
      ...
      const char=getCharById(targetMessage.charId);
      prefillParts.push(`[CHARACTER:${char?.name}]\n`);
    }
    if(rePre) {
      prefillParts[prefillParts.length-1] +=rePre;
    }
  ...
  const handleSwitchVersion=useCallback((msgIdx, alternativeId)=> {
    ...
    if(!message||!message.alts) return;
    const selectedAlt=message.alts.find(alt=> alt.id===alternativeId);
  ...
  }, [characters, charGrps, conversations, curConvId, model, thinkOn, thinkBdg, stats, autoSave, inited]);
  ...
  const loadFromStorage=async ()=> {
    try {
      ...
      if(data) {
        ...
        if(data.thinkOn !==undefined) {
          sThinkOn(data.thinkOn);
        }
        if(data.thinkBdg) {
  ...
  useEffect(()=> {
    if(!inited) return;
    debouncedSave();
  }, [characters, conversations, curConvId, model, thinkOn, thinkBdg, stats, autoSave, inited, debouncedSave]);
  useEffect(()=> {
  ...
  if(!inited) {
    return(
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-indigo-50 to-purple-50"><div className="text-center"><div className="animate-spin rounded-full h-16 w-16 border-b-4 border-indigo-600 mx-auto mb-2"></div><p className="text-gray-600">読み込み中…</p></div></div>
    );
  }
  return(
    <div className="flex flex-col h-screen bg-gradient-to-br from-indigo-50 to-purple-50">
      ...
      {}
      {showSet&&(
        <div className="bg-white border-b border-gray-200 p-3 space-y-3 max-h-96 overflow-y-auto"><div className="flex flex-wrap gap-2"><button
              onClick={()=> newConv()}
              className="flex items-center gap-1 px-3 py-2 bg-indigo-500 text-white rounded hover:bg-indigo-600 text-sm"
            ><Plus size={16} />
              新規会話
            </button><button
              onClick={()=> {
              ...
              className="flex items-center gap-1 px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-300 text-sm"
            ><Download size={16} />
              会話保存
            </button><button
            ...
            ><Upload size={16} />
            ...
            </p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><div><div className="flex items-center justify-between mb-1"><label className="block text-sm text-gray-700">モデル</label><button
                  onClick={fetchModels}
                  disabled={loadMdl}
                  className="text-indigo-600 hover:text-indigo-700 disabled:text-gray-400 p-1"
                  title="モデル一覧を更新"
                ><RefreshCw size={14} className={loadMdl ? 'animate-spin' : ''} /></button></div><select
                value={model}
                onChange={(e)=> setMdl(e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded text-sm"
                disabled={loading||loadMdl}
              >
                {models.length===0 ? (
              ...
              </select></div><div><label className="block text-sm text-gray-700 mb-1">Thinking</label><div className="flex items-center gap-3"><input
                  type="checkbox"
                  checked={thinkOn}
                  onChange={(e)=> sThinkOn(e.target.checked)}
                  className="w-5 h-5"
                  disabled={loading}
                />
                {thinkOn&&(
      ...
      <div className="flex flex-1 overflow-hidden">
        {}
        <div className={`w-64 bg-white border-r border-gray-200 overflow-y-auto p-3 flex-shrink-0  ${showSide ? 'block' : 'hidden lg:block'}`}><div className="flex gap-1 mb-3"><button
            ...
            {currentMessages.length===0 ? (
              ...
              <div className="space-y-1">
                {currentMessages.map((msg, idx)=> {
                  const char=msg.charId ? getCharById(msg.charId) : null;
                  return(
                    <button
                      key={idx}
                        ...
                        {Object.entries(stats.characterAffectionHistory).map(([charId, history])=> {
                          ...
                          return(
                            <div key={charId} className="space-y-1"><div className="flex items-center gap-1 text-xs">
                                {char&&<AvatarDisplay character={char} size="sm" />}
                                <span className="">{char?.name||'不明'}</span></div><svg width={width} height={height} className="bg-white rounded border border-pink-200">
                                {}
                              ...
                              </svg><div className="flex justify-between text-xs text-gray-500"><span>開始</span><span className="text-pink-600">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          {currentMessages.length===0&&curConv&&(
            <div className="text-center text-gray-500 mt-20"><div className="text-6xl mb-2">💬</div><p className="text-lg font-semibold">会話を開始しましょう!</p>
          ...
          {getAllMsgs.length>visMsgCnt&&(
            <div className="text-center py-2"><button
                onClick={()=> sVisCnt(prev=> prev+MSG_INC)}
                className="px-4 py-2 bg-indigo-100 text-indigo-700 rounded hover:bg-indigo-200 text-sm flex items-center gap-2 mx-auto"
              ><ChevronDown size={16} />
          ...
          {getVisibleMessages.map((message, visibleIndex)=> {
            ...
            <div key={actualIndex} ref={(el)=> msgRefs.current[actualIndex]=el}><MessageBubble
              message={message}
              index={actualIndex}
              character={message.charId ? getCharById(message.charId) : null}
              edIdx={edIdx}
              edCont={edCont}
              sEdCont={sEdCont}
              editEmo={editEmo}
          ...
          {loading&&(
            <div className="flex justify-start"><div className="bg-white rounded-2xl rounded-tl-none shadow p-3"><div className="flex items-center gap-3"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-600"></div><span className="text-gray-600 text-sm">考え中...</span></div></div></div>
          )}
          {error&&(
            <div className="bg-red-50 border border-red-200 rounded p-3 flex items-start gap-3"><AlertCircle className="flex-shrink-0 text-red-500" size={20} /><div className="flex-1"><p className="font-semibold text-red-800 text-sm">エラー</p><p className="text-sm text-red-600">{error}</p></div></div>
          )}
      ...
      <div className="bg-white border-t border-gray-200 p-3 space-y-2"><div className="flex gap-2 items-center flex-wrap"><div className="flex gap-1 bg-gray-100 rounded p-1"><button
            ...
            <div className="flex items-center gap-2"><label className="text-xs text-gray-600">次の発言者:</label><select
              ...
              ><option value="">自動</option>
                {curConv.partIds.map(charId=> {
                  const char=getCharById(charId);
                  if(!char) return null;
                  const avatar=char.features.avTyp==='emoji' ? char.features.avatar : '📷';
                  return(
          ...
          /><button
            ...
            className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 disabled:bg-gray-300 flex items-center gap-2 text-sm self-end"
...
const AvatarDisplay=React.memo(({ character, size='md' })=> {
  ...
  if(character.features.avTyp==='image'&&character.features.avatImg) {
    return(
...
const ConfirmDialog=React.memo(({ title, message, confirm, cancel })=> {
  return(
    ...
    ><div className="bg-white rounded shadow-xl max-w-md w-full mx-4"><div className="p-6"><h3 className="text-lg font-bold text-gray-800 mb-2">{title}</h3><p className="text-gray-600 whitespace-pre-line mb-6">{message}</p><div className="flex gap-3 justify-end"><button
              onClick={cancel}
...
const EmojiPicker=({ select, close })=> {
  ...
  return(
    ...
    ><div
      ...
      ><div className="flex items-center justify-between p-3 border-b"><h3 className="text-lg font-bold text-gray-800">絵文字を選択</h3><button
            onClick={(e)=> {
              e.preventDefault();
              e.stopPropagation();
              close();
          ...
          {Object.entries(emojiCategories).map(([key, category])=> (
        ...
        </div><div className="p-3 h-80 overflow-y-auto" onClick={(e)=> e.stopPropagation()}><div className="grid grid-cols-8 gap-2">
            {emojiCategories[activeCategory].emojis.map((emoji, index)=> (
              <button
                ...
                className="text-3xl p-2 hover:bg-gray-100 rounded"
              >
                {emoji}
              </button>
            ))}
...
const ImageCropper=({ imageSrc, crop, cancel })=> {
  ...
  return(
    ...
    ><div
        ...
        onClick={(e)=> e.stopPropagation()}
      ><div className="flex items-center justify-between p-3 border-b"><h3 className="text-lg font-bold text-gray-800">画像をクロップ</h3><button
            onClick={(e)=> {
              e.preventDefault();
              e.stopPropagation();
              cancel();
            ...
            className="p-2 hover:bg-gray-100 rounded"
          ><X size={20} /></button></div><div className="p-3 space-y-4" onClick={(e)=> e.stopPropagation()}><div className="relative"><canvas
              ref={canvasRef}
              width={400}
              height={400}
            ...
            /></div><div className="space-y-2"><label className="block text-sm text-gray-700">
...
const MessageBubble=React.memo(({
  ...
  return(
    <div className={`flex ${
      isNarration ? 'justify-center' : isUser ? 'justify-end' : 'justify-start'
      ...
      } w-full p-3`}><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2">
            {isNarration ? (
              <><FileText size={18} className="text-gray-500" /><span className="text-sm text-gray-600">地の文</span></>
            ) : isUser ? (
        ...
        {showRePre===index&&!isUser&&(
          <div className="mb-3 bg-purple-50 border border-purple-200 rounded p-3"><label className="block text-xs text-purple-700 mb-2">
            ...
            /><div className="flex gap-2"><button
                ...
                className="flex-1 px-3 py-2 bg-purple-500 text-white rounded hover:bg-purple-600 text-xs flex items-center justify-center gap-1"
                disabled={loading}
              ...
              ><RotateCcw size={12} />
                ...
                className="flex-1 px-3 py-2 bg-indigo-500 text-white rounded hover:bg-indigo-600 text-xs flex items-center justify-center gap-1"
                disabled={loading}
        ...
        {message.thinking&&(
          <div className="mb-3 border-l-4 border-yellow-400 bg-yellow-50 p-3 rounded"><div className="flex items-center justify-between mb-2"><span className="text-xs font-semibold text-yellow-700">💭 思考</span><button
                ...
                className="text-yellow-600 hover:bg-yellow-100 p-1 rounded cursor-pointer"
              >
                {(sThink[index] ?? true) ? <EyeOff size={14} /> : <Eye size={14} />}
              </button></div>
            {(sThink[index] ?? true)&&(
        ...
        {edIdx===index ? (
          <div className="space-y-2"><textarea
              value={edCont}
              onChange={(e)=> sEdCont(e.target.value)}
            ...
            {!isNarration&&!isUser&&character&&(character.features.emoOn||character.features.affOn)&&(
              <div className={`gap-3 ${character.features.emoOn&&character.features.affOn ? 'grid grid-cols-2' : 'flex flex-col'}`}>
                {character.features.emoOn&&(
                  <div><label className="block text-xs text-gray-700 mb-1">感情</label><select
                      value={editEmo||''}
                      onChange={(e)=> sEdEmo(e.target.value||null)}
                      className="w-full px-3 py-2 border border-gray-300 rounded text-sm"
                ...
                {character.features.affOn&&(
                  <div><label className="block text-xs text-gray-700 mb-1">好感度 (0-100)</label><input
                      type="number"
                      min="0"
                      max="100"
          ...
          <><pre className="whitespace-pre-wrap font-sans text-gray-800 text-sm leading-relaxed">
            ...
            {}
            {message.alts&&message.alts.length>1&&(
              <div className="mt-3 pt-3 border-t border-gray-200"><div className="flex items-center justify-between"><button
                  ...
                  ><History size={14} /><span>{message.alts.length}つのバージョン</span>
                    {showVers[index] ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                  </button></div>
                {showVers[index]&&(
                  <div className="mt-2 space-y-1">
                    {message.alts.slice().reverse().map((alt, i)=> {
                      const versionNumber=message.alts.length-i;
                      return(
                        <button
                          key={alt.id}
  ...
  return prevProps.message.content===nextProps.message.content &&
         ...
         prevProps.edIdx===nextProps.edIdx &&
...
const ConversationListItem=React.memo(({
  ...
  return(
    ...
    ><div className="flex items-start gap-2 p-2"><button
        ...
        ><div className="flex items-center gap-2 mb-1">
            ...
            {edConvTtl===conversation.id ? (
              <input
                type="text"
                value={edTtl}
                onChange={(e)=> sEdTtl(e.target.value)}
                onKeyDown={(e)=> {
                  if(e.key==='Enter') {
                    updConv(conversation.id, { title: edTtl });
                    sEdConvTtl(null);
            ...
            </span></div></button><div className="flex gap-1 opacity-0 group-hover:opacity-100 flex-shrink-0"><button
            onClick={(e)=> {
              e.stopPropagation();
  ...
  return prevProps.conversation.id===nextProps.conversation.id &&
         ...
         prevProps.edConvTtl===nextProps.edConvTtl &&
         prevProps.edTtl===nextProps.edTtl;
});
const ConversationSettingsPanel=React.memo(({ conversation, characters, update, close })=> {
  ...
  return(
    <div
      className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-3 overflow-y-auto"
      style={{ zIndex: 50 }}
    ><div
        className="bg-white rounded shadow-xl w-full max-w-3xl my-8 flex flex-col"
        style={{ maxHeight: 'calc(100vh-4rem)' }}
        onClick={(e)=> e.stopPropagation()}
      ><div className="bg-white border-b p-3 flex items-center justify-between flex-shrink-0"><h3 className="font-semibold text-xl text-indigo-600 flex items-center gap-2"><Users size={24} />
            会話設定
          </h3><button onClick={close} className="p-2 hover:bg-gray-100 rounded"><X size={20} /></button></div><div className="flex-1 overflow-y-auto p-3 space-y-4" style={{ minHeight: 0 }}><div><label className="block text-sm text-gray-700 mb-1">会話タイトル</label><input
          type="text"
          value={localTitle}
          onChange={(e)=> setLocalTitle(e.target.value)}
          className="w-full px-3 py-2 border border-gray-300 rounded"
        /></div><div><label className="block text-sm text-gray-700 mb-1">
          背景情報・シチュエーション
        </label><textarea
        ...
        /></div><div className="space-y-2"><label className="flex items-center gap-2"><input
            ...
            className="w-4 h-4"
          /><span className="text-sm text-gray-700">地の文を有効化</span></label><p className="text-xs text-gray-500 ml-6">
        ...
        {localNarration&&(
          <div className="ml-6 mt-2 p-3 bg-purple-50 border border-purple-200 rounded"><label className="flex items-center gap-2"><input
                ...
                className="w-4 h-4"
              /><span className="text-sm text-purple-700">AIが自動で地の文を生成</span></label><p className="text-xs text-purple-600 mt-1 ml-6">
              会話の合間に自動的に情景描写や行動描写を挿入します
            </p></div>
        )}
      </div><div><label className="block text-sm text-gray-700 mb-2">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {characters.map(char=> (
              ...
              ><input
                  ...
                  className="w-4 h-4"
                /><AvatarDisplay character={char} size="sm" /><div className="flex-1"><div className="text-sm">{char.name}</div><div className="text-xs text-gray-500">{char.definition.pers}</div></div></label>
            ))}
          </div>
        )}
      </div><div><div className="flex items-center justify-between mb-2"><label className="block text-sm text-gray-700">
            キャラクター間の関係性 ({localRelationships.length}件)
          </label><button
            onClick={addRelationship}
            disabled={localParticipants.length<1}
            className="text-xs px-3 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 disabled:bg-gray-100 disabled:text-gray-400 flex items-center gap-1"
          ><Plus size={14} />
            追加
          </button></div>
      ...
      </div></div><div className="sticky bottom-0 bg-white border-t p-3 flex gap-2 flex-shrink-0"><button
            onClick={handleSave}
...
特に **custPrompt** に詳細な情報を記述し、pers/speakingStyle は簡潔なラベルとして記入してください。`;
  ...
  return(
    ...
    ><div
        ...
        style={{ maxHeight: 'calc(100vh-4rem)' }}
        onClick={(e)=> e.stopPropagation()}
      ><div className="flex items-center justify-between p-3 border-b flex-shrink-0"><div className="flex items-center gap-3"><h2 className="text-xl font-bold text-indigo-600">キャラ管理</h2>
            {editingChar&&(
            ...
            <button
              ...
              className="p-2 hover:bg-gray-100 rounded"
            ><X size={20} /></button></div></div><div className="overflow-y-auto p-3 flex-1" style={{ minHeight: 0 }}>
          {editingChar ? (
            <div className="space-y-3">
              ...
              <div><div className="flex items-center justify-between mb-1"><label className="block text-sm">名前 *</label>
                  {isDerived&&(
                ...
                /></div><div className={`${editingChar.baseCharacterId&&isOverridden(editingChar, 'pers') ? 'bg-yellow-50 border-yellow-200' : ''} border rounded p-3`}><div className="flex items-center justify-between mb-1"><label className="block text-sm">
                    性格
                ...
                /></div><div className={`${editingChar.baseCharacterId&&isOverridden(editingChar, 'speakingStyle') ? 'bg-yellow-50 border-yellow-200' : ''} border rounded p-3`}><div className="flex items-center justify-between mb-1"><label className="block text-sm">
                    話し方
                ...
                /></div><div className="grid grid-cols-2 gap-3"><div className={`${editingChar.baseCharacterId&&isOverridden(editingChar, 'firstPerson') ? 'bg-yellow-50 border-yellow-200' : ''} border rounded p-3`}><div className="flex items-center justify-between mb-1"><label className="block text-sm">
                      一人称
                  ...
                  /></div><div className={`${editingChar.baseCharacterId&&isOverridden(editingChar, 'secondPerson') ? 'bg-yellow-50 border-yellow-200' : ''} border rounded p-3`}><div className="flex items-center justify-between mb-1"><label className="block text-sm">
                      二人称
                  ...
                  /></div></div><div><div className="flex items-center justify-between mb-2"><label className="block text-sm">口癖・決まり文句</label><button
                    onClick={()=> {
              ...
              </div><div className={`${editingChar.baseCharacterId&&isOverridden(editingChar, 'custPrompt') ? 'bg-yellow-50 border-yellow-200' : ''} border rounded p-3`}><div className="flex items-center justify-between mb-1"><label className="block text-sm">
                    カスタムシステムプロンプト
                ...
                /><p className="text-xs text-gray-500 mt-1">
                ...
                </p></div><div><div className="flex items-center justify-between mb-1"><label className="block text-sm">アバター</label>
                  {isDerived&&(
                    <label className="flex items-center gap-1 text-xs text-purple-600"><input
                        type="checkbox"
                    ...
                    onClick={()=> setEditingChar({
                      ...editingChar,
                      features: {...editingChar.features, avTyp: 'emoji'}
                    })}
                    className={`flex-1 px-3 py-2 rounded text-sm   ${
                      editingChar.features.avTyp==='emoji'
                        ? 'bg-indigo-600 text-white'
                  ...
                  >
                    ...
                    onClick={()=> setEditingChar({
                      ...editingChar,
                      features: {...editingChar.features, avTyp: 'image'}
                    })}
                    className={`flex-1 px-3 py-2 rounded text-sm   ${
                      editingChar.features.avTyp==='image'
                        ? 'bg-indigo-600 text-white'
                        : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                    }`}
                    disabled={isDerived&&!editingChar.overrides.avatar}
                ...
                {editingChar.features.avTyp==='emoji' ? (
                  <div><label className="block text-sm text-gray-700 mb-2">絵文字</label><div className="flex items-center gap-2"><div className="flex-1 flex items-center justify-center bg-white border-2 border-gray-300 rounded p-3"><span className="text-5xl">{editingChar.features.avatar||'😊'}</span></div><button
                        onClick={(e)=> {
                          e.preventDefault();
                          e.stopPropagation();
                  ...
                  <div><label className="block text-sm text-gray-700 mb-2">画像</label>
                    {editingChar.features.avatImg ? (
                      <div className="space-y-3"><div className="flex items-center gap-3"><div className="flex-1 flex items-center justify-center bg-white border-2 border-gray-300 rounded p-3"><img
                      ...
                      <div
                        ...
                        onDrop={handleDrop}
                        className={`relative border-2 border-dashed rounded p-8  ${
                          isDragging
                      ...
                      ><div className="flex flex-col items-center justify-center gap-3"><div className="text-4xl">
                            {isDragging ? '📥' : '🖼️'}
                          </div><div className="text-center"><p className="text-sm text-gray-700 mb-1">
                ...
                accept="image}
      {showAutoSetupModal&&(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3"><div className="bg-white rounded shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col"><div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between"><h2 className="text-xl font-bold flex items-center gap-2"><User size={24} className="text-purple-600" />
                AIアシストキャラクター作成
              </h2><button
            ...
            <div className="flex border-b bg-gray-50"><button
              ...
              {autoSetupMode==='template' ? (
                !generatedTemplate ? (
                  <><div className="bg-blue-50 border border-blue-200 rounded p-3"><p className="text-sm text-blue-900"><strong>📋 テンプレート生成:</strong> キャラクター名と作品名を入力すると、WebSearch対応AIで使用するプロンプトとテンプレートを生成します。
                        生成されたプロンプトを Claude.ai などのWebSearch対応AIに入力して、正確なキャラクター設定を作成してください。
                      </p></div><div><label className="block text-sm text-gray-700 mb-2">
                  ...
                  <><div className="bg-green-50 border border-green-200 rounded p-3"><p className="text-sm text-green-900"><strong>✅ プロンプト生成完了:</strong> 以下のプロンプトをコピーして、Claude.ai などのWebSearch対応AIに入力してください。
                      </p></div><div><div className="flex items-center justify-between mb-2"><label className="block text-sm text-gray-700">プロンプト</label><button
                      ...
                      /></div><div><div className="flex items-center justify-between mb-2"><label className="block text-sm text-gray-700">テンプレートJSON</label><div className="flex gap-2"><button
                      ...
                      </p></div><div className="border-t pt-4"><h3 className="text-gray-900 mb-3">📝 次の手順:</h3><ol className="list-decimal list-inside space-y-2 text-sm text-gray-700"><li>上記のプロンプトを <strong>コピー</strong> してください</li><li><strong>Claude.ai</strong> を新しいタブで開く（WebSearch機能が利用可能）</li><li>新しいチャットでプロンプトを貼り付けて送信</li><li>AIが生成したJSON形式の設定をコピー</li><li>このアプリの「<strong>インポート</strong>」機能でJSONを読み込む</li></ol><div className="mt-3 text-xs text-gray-600 bg-blue-50 p-2 rounded">
                ...
                !genCharPrev ? (
                  <><div className="bg-purple-50 border border-purple-200 rounded p-3"><p className="text-sm text-purple-900"><strong>✨ AI生成:</strong> 簡単な説明を入力すると、AIが詳細なキャラクター設定を自動生成します。
                        オリジナルキャラクターの作成に最適です。
                      </p></div><div><label className="block text-sm text-gray-700 mb-2">
                    ...
                    {genErr&&(
                      <div className="bg-red-50 border border-red-200 rounded p-3"><p className="text-sm text-red-900"><strong>エラー:</strong> {genErr}
                        </p></div>
                    )}
                    <div className="flex gap-3 pt-4"><button
                        ...
                        className="flex-1 px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                      >
                  ...
                  <><div className="bg-green-50 border border-green-200 rounded p-3"><p className="text-sm text-green-900"><strong>✅ 生成完了:</strong> キャラクター設定が生成されました。内容を確認して、必要に応じて編集画面で調整してください。
                      </p></div><div className="space-y-3 border rounded p-3 bg-gray-50"><div><label className="block text-sm text-gray-700 mb-1">名前</label><p className="text-base font-semibold">{genCharPrev.name}</p></div><div><label className="block text-sm text-gray-700 mb-1">性格</label><p className="text-sm text-gray-800">{genCharPrev.pers}</p></div><div><label className="block text-sm text-gray-700 mb-1">話し方</label><p className="text-sm text-gray-800">{genCharPrev.speakingStyle}</p></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-sm text-gray-700 mb-1">一人称</label><p className="text-sm text-gray-800">{genCharPrev.firstPerson}</p></div><div><label className="block text-sm text-gray-700 mb-1">二人称</label><p className="text-sm text-gray-800">{genCharPrev.secondPerson}</p></div></div><div><label className="block text-sm text-gray-700 mb-1">背景</label><p className="text-sm text-gray-800">{genCharPrev.background}</p></div>
                      {genCharPrev.phrases&&genCharPrev.phrases.length>0&&(
                        <div><label className="block text-sm text-gray-700 mb-1">決め台詞</label><ul className="list-disc list-inside space-y-1">
                      ...
                      {genCharPrev.custPrompt&&(
                        <div><label className="block text-sm text-gray-700 mb-1">詳細設定（カスタムプロンプト）</label><div className="text-xs text-gray-800 bg-white p-3 rounded border whitespace-pre-wrap max-h-64 overflow-y-auto">
                            {genCharPrev.custPrompt}
                          </div></div>
                      )}
                    </div><div className="flex gap-3 pt-4"><button
